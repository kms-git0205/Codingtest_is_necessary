...
int cntNum(int cur_num, string numbers){
    ...
    if(numbers.size()==1){
        ...
        //cout<<"target(마지막) : "<<cur_num<<endl;
        if(isPrime(cur_num)) cnt++;
        return cnt;
    }
    else{   //문자열 여러개 남은 경우 - 하나하나 더해보기
        ...
        for(int i=0; i<numbers.size(); i++){    //문자열 추가해서 보기
            ...
            //cout<<"numbers_target : "<<numbers_target<<endl;
            //cout<<"i 지우기 : "<<numbers_target[i]<<endl;
            numbers_target.erase(i, 1);   //numbers에서 제거
            //cout<<"after numbers_target : "<<numbers_target<<endl;
